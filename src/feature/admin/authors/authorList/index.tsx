...
const AuthorList = () => {
  ...
  const dataAuthors = authors?.authors
    ...
    },
    {
      ...
      render: (record: any) => (
        <Space size='middle' key={record?._id}>
          <Link to={`/admin/authors/${record?._id}/update`} className='bg-sky-400 text-white px-4 py-2 rounded-md '>
            <EditOutlined />
          </Link>
          <Popconfirm
            placement='topRight'
            title='Delete the authors'
            description='Are you sure to delete this authors?'
  ...
  return (
    <div>
      ...
      <div className='text-red-500'>
        <Link to={'/admin/authors/trash'} className='text-2xl p-2 bg-slate-200 rounded-md'>
          <DeleteOutlined />
        </Link>
      </div>
    </div>
